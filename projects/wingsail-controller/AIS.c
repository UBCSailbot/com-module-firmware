...
void AIS__init(AIS* self, uint8_t inputData[]) {
    for (uint8_t i = 0; inputData[i] != '\0' && i > MAX_LENGTH; i++) {
        if (inputData[i] <= 87) {
            self->sixBitData[i] = inputData[i] - 48;
        }
        else if (inputData[i] <= 119) {
            self->sixBitData[i] = inputData[i] - 56;
        }
        else {
            self->sixBitData[i] = 255;
        }
    }
}
...
uint8_t AIS__getMessageID(AIS* self) {
    return self->sixBitData[0];
}

uint8_t AIS__getRepeatIndicator(AIS* self) {
    return (self->sixBitData[1] >> 4) & 3;
}
...
uint8_t AIS__getNavigationalStatus(AIS* self) {
    if (self->sixBitData[0] <= 3 && self->sixBitData[0] != 0) {
        return self->sixBitData[6] & 15;
    }
    else {
        return UINT8_MAX;
    }
...
uint16_t AIS__getSpeedOverGround(AIS* self) {
    if (self->sixBitData[0] != 0) {
        if (self->sixBitData[0] <= 3) {
            return getBinaryBits(self->sixBitData, 50, 59);
        }
        else if (self->sixBitData[0] == 18 || self->sixBitData[0] == 19) {
...
uint8_t AIS__getPositionalAccuracy(AIS* self) {
    if (self->sixBitData[0] != 0) {
        if (self->sixBitData[0] <= 3) {
            return self->sixBitData[10] >> 5;
        }
        else if (self->sixBitData[0] == 18 || self->sixBitData[0] == 19) {
            return (self->sixBitData[9] >> 3) & 1;
        }
    }
    return UINT8_MAX;
}
...
uint32_t AIS__getCommunicationState(AIS* self) {
    if (self->sixBitData[0] != 0 && (self->sixBitData[0] <= 3 || self->sixBitData[0] == 18)) {
...
uint8_t AIS__getVersionIndicator(AIS* self) {
    if (self->sixBitData[0] == 5) {
        return (self->sixBitData[6] >> 2) & 3;
    }
    return UINT8_MAX;
}
...
bool AIS__getCallSign(AIS* self, uint8_t output[8]) {
    ...
    else if (self->sixBitData[0] == 24 && (self->sixBitData[6] & 12) == 4) {
...
bool AIS__getName(AIS* self, uint8_t output[21]) {
    ...
    else if (self->sixBitData[0] == 24 && (self->sixBitData[6] & 12) == 0) {
...
uint8_t AIS__getDimensionD(AIS* self) {
    if (self->sixBitData[0] == 5) {
        return self->sixBitData[44];;
    }
    else if (self->sixBitData[0] == 19) {
        return getBinaryBits(self->sixBitData, 295, 300);
    }
    else if (self->sixBitData[0] == 24 && (self->sixBitData[6] & 12) == 4) {
        return self->sixBitData[26];
    }
    return UINT8_MAX;
}
...
uint8_t AIS__getDimensionC(AIS* self) {
    if (self->sixBitData[0] == 5) {
        return self->sixBitData[43];
    }
    else if (self->sixBitData[0] == 19) {
        return getBinaryBits(self->sixBitData, 289, 294);
    }
    else if (self->sixBitData[0] == 24 && (self->sixBitData[6] & 12) == 4) {
        return self->sixBitData[25];
...
uint8_t AIS__getPositionFixingDevice(AIS* self) {
    if (self->sixBitData[0] == 5) {
        return (self->sixBitData[45] >> 2) & 15;
    }
    else if (self->sixBitData[0] == 19) {
        return (self->sixBitData[50] >> 1) & 15;
    }
    else if (self->sixBitData[0] == 24 && (self->sixBitData[6] & 12) == 4) {
        return (self->sixBitData[27] >> 2) & 15;
    }
    return UINT8_MAX;
}
...
uint8_t AIS__getMessageAOrB(AIS* self) {
    if (self->sixBitData[0] == 24) {
        if ((self->sixBitData[6] & 12) == 0) {
            return 0;
        }
        else if ((self->sixBitData[6] & 12) == 4) {
            return 1;
        }
    }
    return UINT8_MAX;
...
bool AIS__getVendorID(AIS* self, uint8_t output[8]) {
    if (self->sixBitData[0] == 24 && (self->sixBitData[6] & 12) == 4) {
