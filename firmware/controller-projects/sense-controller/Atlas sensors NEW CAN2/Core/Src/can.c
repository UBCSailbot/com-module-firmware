 ...
 * 			Format = {ID_3, 1D_2, 1D1, ID_0, DataLength, Data, ..., Data} (little endian 32 bit ID)
 * @note    The function prioritizes FIFO0 over FIFO1 if both have data.
 */
/**
 * @brief   Retrieves the oldest received CAN message from the queue.
 ...
 *          automatically discarded to make room for the new message. This ensures
 *          the most recent messages are always available.
 */
static void CAN_EnqueueFrame(uint32_t id, uint8_t len, const uint8_t *data) {
