...
int main(void)
{
  ...
  while (1)
  {
	...
	/* Receiving */
//	uint8_t RxBuffer[69]; memset(RxBuffer, 0, 69);
//	while (CAN_Receive(RxBuffer) != HAL_OK) { continue;}
//	rx_cnt++;
//	HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_SET);
//	uint32_t id =  (uint32_t)RxBuffer[0] | ((uint32_t)RxBuffer[1] << 8) | ((uint32_t)RxBuffer[2] << 16) | ((uint32_t)RxBuffer[3] << 24);
//	uint8_t len = RxBuffer[4];
//	uint8_t localRxbuffer[4]; //If you want a Data only buffer, make it locally
//	for (int i = 0; i < len; i++) {
//	    localRxbuffer[i] = RxBuffer[5+i];
//	}
//	printf("Msg #: %d; ID: 0x%04X; len: 0x%04X \r\n", rx_cnt, id, len);
